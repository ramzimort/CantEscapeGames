...
const bool UserManager::GetSettingBool(const std::string& parent, const std::string& setting)
{
	if (HasSetting(parent, setting))
		return m_settings[parent][setting].GetBool();
	return false;
}
...
{
	if (HasSetting(parent, setting))
		return m_settings[parent][setting].GetInt();
	return false;
}
...
{
	if (HasSetting(parent, setting))
		return m_settings[parent][setting].GetDouble();
	return 0.0;
}
...
{
	if (HasSetting(parent, setting))
		return m_settings[parent][setting].GetString();
	return "";
}
